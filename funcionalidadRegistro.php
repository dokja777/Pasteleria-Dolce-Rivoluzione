...
if (isset($_POST['NUMERO_DOC']) && isset($_POST['Correo']) && isset($_POST['Nombre']) && isset($_POST['Apellido']) && isset($_POST['Telefono']) && isset($_POST['Contraseña']) && isset($_POST['RContraseña'])) {
    ...
    if(empty($NUMERO_DOC)){
        header("Location: registro.php?error=El número de documento es requerido&$datosUsuario");
        exit();
    } elseif (empty($Correo)){
        header("Location: registro.php?error=El correo es requerido&$datosUsuario");
        exit();
    } elseif (empty($Nombre)){
        ...
        header("Location: registro.php?error=El número de teléfono es requerido&$datosUsuario");
        exit();
    } elseif (empty($Contraseña)){
        header("Location: registro.php?error=La contraseña es requerida&$datosUsuario");
        exit();
    } elseif (empty($RContraseña)){
        header("Location: registro.php?error=Tiene que repetir la contraseña&$datosUsuario");
        exit();
    } elseif($Contraseña !== $RContraseña){
        header("Location: registro.php?error=Las contraseñas no coinciden&$datosUsuario");
        exit();
    } else {
        ...
        if(mysqli_num_rows($query_check_documento) > 0){
            header("Location: registro.php?error=El número de documento ya existe&$datosUsuario");
            exit();
        } else {
            ...
            if($query2){
                header("Location: registro.php?success=Usuario creado con éxito&$datosUsuario");
                exit();
            } else {
                header("Location: registro.php?error=Ocurrió un error al registrar el usuario&$datosUsuario");
                exit();
            }
        }
    }
